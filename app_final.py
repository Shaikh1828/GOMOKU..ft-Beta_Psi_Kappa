...
# $env:FLASK_APP = "app_final.py"; flask run
#    1 # In PowerShell
#    2 Remove-Item -Recurse -Force .venv
#    3 python -m venv .venv
#    4 .venv\Scripts\Activate.ps1
#    5 pip install -r requirements.txt  # If you have a requirements.txt
...
#   11 flask
#   12  flask run
#   13 $env:FLASK_APP = "app_final.py"; flask run

from flask import Flask, redirect, request, render_template
import json

from gameRunner_final import GameRunner
...
def restart(level):
    ...
    if player_index not in [-1, 1]:
        ...
        status = 'OK'
        game_runner.restart(player_index, level)  # Pass the level
